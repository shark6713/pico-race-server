...
io.on("connection", (socket) => {
  ...
  const enterGame = (room: GameState, displayName?: string) => {
  ...
  socket.on("findGame", (data: { displayName?: string } = {}) => {
    ...
    setTimeout(() => {
      ...
      enterGame(foundRoom, data.displayName);
    }, 1500); 
  });
  ...
  socket.on("acceptInvite", (roomId, data) => {
     if (rooms[roomId]) {
         enterGame(rooms[roomId], data?.displayName || onlineUsers[socket.id]?.displayName);
     }
  });
